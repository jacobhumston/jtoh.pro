(() => {
    ...
    const getBrowserFingerprint = async ({ hardwareOnly = true, debug = false } = {}) => {
        ...
        const { width, height, colorDepth, pixelDepth } = enableScreen ? _window.screen : {}; // undefined will remove this from the stringify down here
        const timezoneOffset = new Date().getTimezoneOffset();
        const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
        const touchSupport = 'ontouchstart' in _window;
        const devicePixelRatio = _window.devicePixelRatio;
    ...
    const getCanvasID = (debug) => {
        try {
            ...
            if (debug) {
                _document.body.appendChild(canvas);
            } else {
    ...
    const getWebglInfo = () => {
        try {
            const ctx = _document.createElement('canvas').getContext('webgl');
    ...
    const getAudioInfo = () => {
        try {
            const OfflineAudioContext = _window.OfflineAudioContext || _window.webkitOfflineAudioContext;
            const length = 44100;
    ...
    const getAudioID = async (debug) => {
        try {
            ...
            if (debug) {
                ...
                const audioElement = _document.createElement('audio');
                audioElement.controls = true;
                audioElement.src = audioURL;
                _document.body.appendChild(audioElement);
            }
    ...
    _window.func.me = getBrowserFingerprint;
})();
