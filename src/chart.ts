...
import { capitalizedGameNamesArray, gameChartColors, gameNamesArray, type gameNames } from './shared/gamelist';
...
export function charts(app: Hono) {
    ...
    app.get('/api/charts/card-requests', async (context) => {
        const data: { [key: string]: Array<{ date: string; requests: number }> } = {};
        const datasets: any = [];
        ...
        for (const game of gameNamesArray) {
            ...
            // @ts-ignore-next-line
            for await (const [key, value] of statsDB[game].iterator()) {
                data[game].push({ date: key, requests: value });
        ...
        for (const game of gameNamesArray) {
            for (const date of allDates) {
                if (!data[game].find((x) => x.date === date)) {
                    data[game].push({ date, requests: 0 });
                }
            }

            data[game].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

            datasets.push({
                label: capitalizedGameNamesArray[gameNamesArray.indexOf(game)],
                data: data[game].map((x) => x.requests),
                fill: false,
                tension: 0.1,
                backgroundColor: gameChartColors[game],
                borderColor: gameChartColors[game],
                pointRadius: 0
            });
        }
        ...
        const chart = new Chart(canvas, {
            ...
            data: {
                labels: allDates.map((x) =>
                    new Date(x.replaceAll('-', '/')).toLocaleString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric'
                    })
