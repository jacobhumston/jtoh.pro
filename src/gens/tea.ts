import { convertSVGTextToPath, createCanvas, loadImage } from '@napi-rs/canvas';
import { centerText, colorText, drawIconWithText, drawRoundedRect, drawRoundedRectv2, roundedRect } from '../util';
import type { TowerDataTEA } from '../type';
import { v4 } from 'uuid';
...
export default function teaGen(app: Hono) {
    app.get('/tea/:user', createRateLimitMiddleware({ seconds: 10 }, 5), async (context) => {
        ...
        if (data === undefined) {
            ...
            const cached: TowerDataTEA | undefined = towerstatsCache.get(`${data.id}-tea`);
            let towerStats: TowerDataTEA | undefined =
                cached ??
                (await fetch(`https://api.towerstats.com/api/tea`, {
                    ...
                    .then((res) => {
                        ...
                        if (res.ok) return res.json() as Promise<TowerDataTEA>;
                        return undefined;
                    })
                    .catch(() => undefined));
            ...
            if (towerStats !== undefined && cached === undefined) {
                towerstatsCache.set(`${data.id}-tea`, towerStats);
            }
            ...
            */
            ...
            if (towerStats !== undefined) {
                updateCardRequestCount('tea', data).catch(() => undefined);
                await updateSkillPoints('tea', data, towerStats.skill_points).catch(() => undefined);
                await updateTowerCount('tea', data, towerStats.completed_towers).catch(() => undefined);
                ...
                for (const [key, string] of Object.entries(towerStats.difficulties)) {
                    if (towerStats.difficulty_progress[string] === undefined) {
                        delete towerStats.difficulties[key];
                        delete towerStats.difficulty_colors[string];
                        delete towerStats.difficulty_colors_outlines[string];
                    }
                }
                ...
                */
                ...
                })(await getPlaceInLeaderboard(skillPointsDB, 'tea', data).catch(() => null));
                const spTotal =
                    `out of ${formatter.format(await getTotalInLeaderboard(skillPointsDB, 'tea'))}`.replaceAll(
                        ' ',
                        '_'
                    );
                colorText(
                ...
                */
                ...
                const completedTowersRank = ((value: number | null) => {
                    if (value === null) return 'N/A';
                    return `#${formatter.format(value)}`;
                })(await getPlaceInLeaderboard(towerCountDB, 'tea', data).catch(() => null));
                const completedTowerTotal =
                    `out of ${formatter.format(await getTotalInLeaderboard(towerCountDB, 'tea'))}`.replaceAll(' ', '_');
                colorText(
                    ctx,
                ...
                updateRequestCount('tea').catch(() => undefined);
            } else {
                ctx.textAlign = 'left';
                ctx.fillStyle = 'white';
                ctx.font = 'bold 25px Poppins, Twemoji';
            ...
            */
            ...
            drawRoundedRectv2(
                ctx,
                700 - (ctx.measureText(`jtoh.pro/tea/${data.name}`).width + 55),
                -10,
                ctx.measureText(`jtoh.pro/tea/${data.name}`).width + 18,
                35,
            ...
            ctx.textAlign = 'left';
            ctx.fillText(
                `jtoh.pro/tea/${data.name}`,
                700 - (ctx.measureText(`jtoh.pro/tea/${data.name}`).width + 55) + 10,
                15
            );
            ...
            ctx.textAlign = 'left';
            const cacheTimeLeft = towerstatsCache.getRemainingTTL(`${data.id}-tea`);
    ...
    app.get('/tea/embed/:user', async (context) => {
        const providedUser: string = context.req.param('user').slice(0, 20);
        const data = await parseRobloxAccount(context);
            ...
            return context.html(
                `<html>
                    <head> <!-- ${new Date().toISOString()} --!> 
                        ...
                        <meta property="og:description" content="Viewing @${data.name}'s The Eternal Abyss stats. Click the link above to view more stats.">
                        <meta property="og:image" content="${new URL(context.req.url).origin}/tea/${data.name}?nocache=${v4()}">
                        <meta property="og:type" content="image"><meta property="og:url" content="https://towerstats.com/tea?username=${data.name}">
                        <meta property="twitter:card" content="summary_large_image">
                        <meta http-equiv="refresh" content="0; url=https://towerstats.com/tea?username=${data.name}" />
                        <style>
    ...
    app.get('/t/:user', async (context) => {
        return context.redirect('/tea/' + context.req.param('user'));
    });

    app.get('/tea/e/:user', async (context) => {
        return context.redirect('/tea/embed/' + context.req.param('user'));
    });
}
