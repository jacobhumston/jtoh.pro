...
import type { TowerDataOToH } from '../type';
import { v4 } from 'uuid';
...
export default function otohGen(app: Hono) {
    app.get('/otoh/:user', async (context) => {
        ...
        if (data === undefined) {
            ...
            const cached: TowerDataOToH | undefined = await towerstatsCache.get(`${data.id}-otoh`);
            let towerStats: TowerDataOToH | undefined =
                cached ??
                (await fetch(`https://api.towerstats.com/api/otoh`, {
                    method: 'POST',
                    ...
                    .then((res) => {
                        ...
                        if (res.ok) return res.json() as Promise<TowerDataOToH>;
                        return undefined;
                    })
                    .catch(() => undefined));
            ...
            if (towerStats !== undefined && cached === null) {
                await towerstatsCache.set(`${data.id}-otoh`, towerStats);
            }
            ...
            */
            ...
            if (towerStats !== undefined) {
                updateCardRequestCount('otoh', data).catch(() => undefined);
                //await updateSkillPoints('otoh', data, towerStats.skill_points).catch(() => undefined);
                await updateTowerCount('otoh', data, towerStats.completed_towers).catch(() => undefined);
                ...
                if (
                    ...
                    const difficultyColor =
                        towerStats.difficulty_colors[towerStats.difficulties[parseInt(raw.split('.')[0]) + 1]];
                    colorText(
                        ctx,
                ...
                if (
                    ...
                    for (const [key, string] of Object.entries(towerStats.difficulties)) {
                        ...
                        if (!towerStats.difficulty_progress[string]) continue;
                    ...
                    difficultyOrder.forEach((difficulty, index) => {
                        ...
                        if (!difficultyAmount || !difficultyColor) return;
                ...
                drawIconWithText(ctx, 'inventory', '18px', 18, 'Completed Towers', '#f8f8f8', 60, 165);
                ...
                */
                ...
                })(await getPlaceInLeaderboard(skillPointsDB, 'otoh', data).catch(() => undefined));
                const spTotal =
                    `out of ${formatter.format(await getTotalInLeaderboard(skillPointsDB, 'otoh'))}`.replaceAll(
                        ' ',
                        '_'
                    );
                colorText(
                ...
                */
                //ctx.fillText(`${Object.keys(towerStats)} Areas Completed`, 20, 205);

                ctx.textAlign = 'left';
                ctx.fillStyle = '#f8f8f8';
                ctx.font = 'bold 15px Poppins, Twemoji';
                drawIconWithText(ctx, 'trophy', '18px', 18, 'Completed Towers Rank', '#f8f8f8', 380, 165);
                ...
                const completedTowersRank = ((value: number | null) => {
                    if (value === null) return 'N/A';
                    return `#${formatter.format(value)}`;
                })(await getPlaceInLeaderboard(towerCountDB, 'otoh', data).catch(() => undefined));
                const completedTowerTotal =
                    `out of ${formatter.format(await getTotalInLeaderboard(towerCountDB, 'otoh'))}`.replaceAll(
                        ' ',
                        '_'
                    );
                colorText(
                ...
                updateRequestCount('otoh').catch(() => undefined);
            } else {
                ctx.textAlign = 'left';
                ctx.fillStyle = 'white';
                ctx.font = 'bold 25px Poppins, Twemoji';
            ...
            */
            ...
            drawRoundedRectv2(
                ctx,
                700 - (ctx.measureText(`jtoh.pro/otoh/${data.name}`).width + 55),
                -10,
                ctx.measureText(`jtoh.pro/otoh/${data.name}`).width + 18,
                35,
            ...
            ctx.textAlign = 'left';
            ctx.fillText(
                `jtoh.pro/otoh/${data.name}`,
                700 - (ctx.measureText(`jtoh.pro/otoh/${data.name}`).width + 55) + 10,
                15
            );
            ...
            ctx.textAlign = 'left';
            const cacheTimeLeft = ((await towerstatsCache.ttl(`${data.id}-otoh`)) ?? 0) - Date.now();
    ...
    app.get('/otoh/embed/:user', async (context) => {
        const providedUser: string = context.req.param('user').slice(0, 20);
        const data = await parseRobloxAccount(context);

        if (data === undefined) {
            return context.redirect(`/otoh/${providedUser}`);
        } else {
            return context.html(
                `<html>
    ...
    app.get('/otoh/e/:user', async (context) => {
        return context.redirect('/otoh/embed/' + context.req.param('user'));
    });
}
